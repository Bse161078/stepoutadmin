...
import {getReservation,addReservation,deleteReservation,updateReservation} from '../backend/services/reservationService'
    ...
    constructor(props) {
       ...
       // this.handleInputChange = this.handleInputChange.bind(this);
          ...
          async handleDelete(deleteId){
            ...
            try{
              const res = await deleteReservation(deleteId)
              this.setState({
                loading:false,
                snackBarMessage:" Reservation Deleted Successfully",
                snackBarVariant:"success",
                showSnackBar:true
              })
            ...
            catch(e)
            {
              this.setState({
                loading:false,
                snackBarMessage: "Couldn't Delete Reservation",
                snackBarVariant:"success",
                showSnackBar:true
              })
            }
          }
          async handleReservation(event,action)
          {
           this.state.loading=true
            const tempEvent = {
             ...event,
       ...
       {
               try{
                 const res = await addReservation(tempEvent)
                   this.setState({
                     loading:false,
                     snackBarMessage:" Reservation Created",
                     snackBarVariant:"success",
                     showSnackBar:true
                   })
                   this.fetchReservation()
               ...
               catch(err)
               {
                 ...
                 this.setState({
                   loading:false,
                   snackBarMessage:" Reservation Error",
               ...
               {
                try{
                  const res = await updateReservation(event.event_id,event)
                    this.setState({
                      loading:false,
                      snackBarMessage:"Reservation Updated",
                      snackBarVariant:"success",
                      showSnackBar:true
                    })
                    this.fetchReservation()
                ...
                catch(err)
                {
                  ...
                  this.setState({
                    loading:false,
                    snackBarMessage:"Update Reservation Error",
                    snackBarVariant:"error",
                    showSnackBar:true
                  })
                }
            ...
            console.log("responsefilterreservation",res)
              const reservations=(res).map((reservation,index)=>{
                  ...
                  return{
                                ...
                                title: reservation.title?reservation.title:"Reservation # "+(++index),
                                start: new Date(moment(reservation.start).format("YYYY/M/D 01:00")),
                                end: new Date( moment(reservation.end).format("YYYY/M/D 24:00")),
                  }
              });
              ...
              const tripopt = trips.map((trip,index)=>{
                ...
                const res =  [
                      ...
                      },
                      {
                        name:"attendies",
                        type:"input",
                        config:{label:"Attendies",required:false}
                      }
                  ]   
                  return res;
              }
        ...
        }
        {
          ...
          <h2>Reservations</h2>
          </div>
              {
                ...
                fields={this.Field()}
                events={reservations}
                onConfirm={this.handleReservation.bind(this)}
